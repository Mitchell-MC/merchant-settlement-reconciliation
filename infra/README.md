...
**Correction (verified 2026-07-11): do NOT run `terraform apply -var-file=environments/prod.tfvars` against this directory as-is.** It was previously assumed this would be safe because the catalog name differs — that's wrong. This directory has a single Terraform state file tracking a single set of resource addresses (`databricks_catalog.this`, `databricks_schema.bronze`, etc.). Feeding it `prod.tfvars` doesn't provision a second, parallel environment; it changes the `catalog_name`/etc. inputs of the *already-applied dev resources*, which Terraform resolves as "replace this resource" — i.e. destroy and recreate. A real `terraform plan -var-file=environments/prod.tfvars` against the live state showed **11 resources to destroy** (including `databricks_schema.bronze/silver/gold/ops` and both Bronze volumes) and 12 to add. Applying it would have dropped every schema and all Bronze/Silver/Gold data in the live dev catalog.

To actually stand up an isolated prod environment, dev and prod need **separate Terraform state** — either a Terraform workspace per environment (`terraform workspace new prod`) or a separate state backend/key per environment — not just a different `-var-file` against the same state. That's an unimplemented design change, not a one-line fix. Until then, prod stays documented-only, and any `terraform plan -var-file=environments/prod.tfvars` run against this directory should be treated as informational only — read the plan output for "forces replacement" before ever considering `apply`, same lesson as the `storage_root` near-miss below.
