...
- **CI gate** ([.github/workflows/ci.yml](.github/workflows/ci.yml)) — runs on every push/PR: `ruff` lint, `pytest` with a coverage ratchet (`--cov-fail-under`, config in [pyproject.toml](pyproject.toml)), `terraform validate`, and `dbt parse`. A live `dbt build` against the dev database runs on PRs.
- **CD** ([.github/workflows/cd.yml](.github/workflows/cd.yml)) — runs a `dbt build` smoke test against the live database behind a two-part gate (`CD_DEPLOY_ENABLED` + the `SNOWFLAKE_CI_PRIVATE_KEY` secret), then a required-reviewer approval before the daily schedule is activated. Infrastructure is applied via Terraform **separately** (state is local, not in CI) — a deliberate scope, see [cd.yml](.github/workflows/cd.yml) header.
