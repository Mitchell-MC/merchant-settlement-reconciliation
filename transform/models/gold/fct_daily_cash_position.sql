...
{#- Snowflake's default CREATE OR REPLACE TABLE drops existing grants on
   rebuild -- copy_grants preserves the Terraform-managed
   treasury_viewers/bi_consumers SELECT grants (infra_snowflake/grants.tf)
   across every dbt run. Confirmed by testing: without this, a single
   rebuild silently wiped those grants. #}
