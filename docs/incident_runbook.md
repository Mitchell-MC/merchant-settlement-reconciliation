...
- **Dead-man's-switch** — `transform/tests/assert_pipeline_not_silent.sql`, run by `.github/workflows/snowflake_heartbeat.yml` on an **independent** 6-hourly schedule. It fails when the last logged run is older than `pipeline_heartbeat_threshold_hours` (26h), i.e. when the pipeline has gone quiet — the one check the daily job structurally cannot perform on itself.
- **Source freshness gate** — the daily workflow runs `dbt source freshness` *before* build; stale/missing operational Bronze fails the job loudly instead of publishing a green Gold built on old data (`transform/models/bronze/_bronze__sources.yml`).
- **Failure + hang notifications** — the daily workflow's `timeout-minutes: 60` ceiling catches a hung run that never errors, and its failure step posts to `ALERT_WEBHOOK_URL` (`.github/workflows/snowflake_daily.yml`).
...
- Is the heartbeat itself monitored? It runs on GitHub Actions against the same Snowflake warehouse it watches, so a total outage of either silences the watcher too. The stated next step is a fully **external** heartbeat (an off-platform cron hitting the SQL API) — documented, not yet built (`.github/workflows/snowflake_heartbeat.yml` header).
- Which control failed? `.github/workflows/snowflake_daily.yml` carries the freshness gate, the timeout, and the failure alert; `.github/workflows/snowflake_heartbeat.yml` carries the independent dead-man's-switch. Both are backed by the same `assert_pipeline_not_silent` test and `ops.pipeline_heartbeat` model, so a detection gap in one is a gap in the other.
- Were the alert channels actually populated, or still empty? An unset `ALERT_WEBHOOK_URL` secret is a silent failure waiting to happen — GitHub's native workflow-failure email is the only backstop until it's set.
